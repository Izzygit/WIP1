...
static void apply_inputtilt(data *d){ // Input Tiltback
	...
	int truncanglelimit = d->tnt_conf.inputtilt_angle_limit; // Cast the angle limit to remove the decimals
	if (d->tnt_conf.inputtilt_angle_limit - truncanglelimit > 0) { // If we have any decimals allow sticky tilt
		float stickytilt_val1 = 3.0; // Value that defines where tilt will stick for both nose up and down. Can be made UI input later.
		float stickytilt_val2 = 6.0; // Value of 0 or above max disables. Max value <=  d->tnt_conf.inputtilt_angle_limit. 
...
static void check_surge(data *d, float new_pid_value){
	...
	d->surge_startanglespeed = 50; //fmaxf(10, (float)d->tnt_conf.turntilt_start_erpm/10); 	//Nose speed that can initiate a surge
	d->surge_difflimit = 3; //fmaxf(0.1, d->tnt_conf.turntilt_speed);				//Pitch required at start angle speed to initiate surge
	d->surge_currentmargin = 1.3; //fmaxf(0.5, (float)d->tnt_conf.turntilt_erpm_boost/100);	//Higher current margin ends surge later

	//Counter for high nose angle speed
	if (d->differential * SIGN(d->erpm) > d->surge_startanglespeed / d->tnt_conf.hertz){
...
static float apply_rollkp(data *d){
	float m;
	float b;
	float rollkp_mod;
	float kp_min;
	float scale_angle_min;
	float scale_angle_max;
	float kp_max;

	//Determine the correct kp to use based on abs_roll_angle
	if (SIGN(d->proportional) != SIGN(d->erpm)) { //Braking
		if (d->abs_roll_angle > d->tnt_conf.brkroll3  && d->tnt_conf.brkrollkp3>0 && d->tnt_conf.brkrollkp2>0) {
			kp_min = -d->tnt_conf.brkrollkp3;
			kp_max = -d->tnt_conf.brkrollkp3;
			scale_angle_min = d->tnt_conf.brkroll3;
			scale_angle_max = 90;
		} else if (d->abs_roll_angle > d->tnt_conf.brkroll2  && d->tnt_conf.brkrollkp3>0 && d->tnt_conf.brkrollkp2>0) {
			kp_min = -d->tnt_conf.brkrollkp2;
			kp_max = -d->tnt_conf.brkrollkp3;
			scale_angle_min = d->tnt_conf.brkroll2;
			scale_angle_max = d->tnt_conf.brkroll3;
		} else if (d->abs_roll_angle > d->tnt_conf.brkroll1  && d->tnt_conf.brkrollkp2>0) {
			kp_min = -d->tnt_conf.brkrollkp1;
			kp_max = -d->tnt_conf.brkrollkp2;
			...
			kp_max = 0;
			scale_angle_min = 0;
			scale_angle_max = 1; //Any value but zero can be used because kp_min=kp_max
		}
		if (d->erpm < 750) { // If we want to actually stop at low speed reduce kp to 0
			kp_min = 0;
			kp_max = 0;
			scale_angle_min = 0;
			scale_angle_max = 1; //Any value but zero can be used because kp_min=kp_max
		...
		if (d->abs_roll_angle > d->tnt_conf.roll3  && d->tnt_conf.rollkp3>0 && d->tnt_conf.rollkp2>0) {
			kp_min = d->tnt_conf.rollkp3;
			kp_max = d->tnt_conf.rollkp3;
			scale_angle_min = d->tnt_conf.roll3;
			scale_angle_max = 90;
		} else if (d->abs_roll_angle > d->tnt_conf.roll2  && d->tnt_conf.rollkp3>0 && d->tnt_conf.rollkp2>0) {
			kp_min = d->tnt_conf.rollkp2;
			kp_max = d->tnt_conf.rollkp3;
			scale_angle_min = d->tnt_conf.roll2;
			scale_angle_max = d->tnt_conf.roll3;
		} else if (d->abs_roll_angle > d->tnt_conf.roll1  && d->tnt_conf.rollkp2>0) {
			kp_min = d->tnt_conf.rollkp1;
			kp_max = d->tnt_conf.rollkp2;
	...
	if (scale_angle_min == scale_angle_max || scale_angle_max < scale_angle_min) { //Check that the angles make sense. Use kp_min if they do not.
		rollkp_mod = kp_min;
	} else if (fabsf(kp_max) < fabsf(kp_min)) { // Check that kp_max is bigger than kp_min
		kp_max = kp_min;
	} else {
		...
		rollkp_mod = fmaxf(kp_min, fminf(kp_max, m * fabsf(d->prop_smooth) + b)); // scale kp between min and max with the caluclated equation
	}
	d->debug11 = rollkp_mod;
	return rollkp_mod;
}
...
static float apply_kp(data *d) {
	...
	//Determine the correct kp to use based on prop_smooth
	if (d->prop_smooth > d->tnt_conf.angle4 && d->tnt_conf.kp4>0 && d->tnt_conf.kp3>0 && d->tnt_conf.kp2>0 && d->tnt_conf.kp1>0)) {
		kp_min = d->tnt_conf.kp4;
		kp_max = d->tnt_conf.kp4;
		scale_angle_min = d->tnt_conf.pitch4;
		scale_angle_max = 90; //Any value but zero can be used because kp_min=kp_max
	} else if (d->prop_smooth > d->tnt_conf.angle3 && d->tnt_conf.kp4>0  && d->tnt_conf.kp3>0 && d->tnt_conf.kp2>0 && d->tnt_conf.kp1>0) {
		kp_min = d->tnt_conf.kp3;
		kp_max = d->tnt_conf.kp4;
		scale_angle_min = d->tnt_conf.pitch3;
		scale_angle_max = d->tnt_conf.pitch4;
	} else if (d->prop_smooth > d->tnt_conf.angle2 && d->tnt_conf.kp3>0 && d->tnt_conf.kp2>0 && d->tnt_conf.kp1>0) {
		kp_min = d->tnt_conf.kp2;
		kp_max = d->tnt_conf.kp3;
		scale_angle_min = d->tnt_conf.pitch2;
		scale_angle_max = d->tnt_conf.pitch3;
	} else if (d->prop_smooth > d->tnt_conf.angle2 && d->tnt_conf.kp2>0 && d->tnt_conf.kp1>0) {
		kp_min = d->tnt_conf.kp1;
		kp_max = d->tnt_conf.kp2;
		scale_angle_min = d->tnt_conf.pitch1;
		scale_angle_max = d->tnt_conf.pitch2;
	} else if (d->prop_smooth > d->tnt_conf.angle1 && d->tnt_conf.kp1>0) {
		kp_min = d->tnt_conf.kp0;
		kp_max = d->tnt_conf.kp1;
		scale_angle_min = 0;
		scale_angle_max = d->tnt_conf.pitch1;
	} else {
		kp_min = d->tnt_conf.kp0;
		kp_max = d->tnt_conf.kp0;
		scale_angle_min = 0;
		scale_angle_max = 1; //Any value but zero can be used because kp_min=kp_max
	}
...
static void tnt_thd(void *arg) {
	...
	while (!VESC_IF->should_terminate()) {
		...
		case (RUNNING_UPSIDEDOWN):
			...
			d->pid_prop = apply_kp(d) * d->proportional;

			new_pid_value = d->pid_prop;
			...
			if (d->start_counter_clicks == 0) {
				...
				d->pid_mod = d->tnt_conf.kp2 * rate_prop;
				
				// Apply Turn Boost
				d->pid_mod += (1 + apply_rollkp(d)) * new_pid_value;
...
static void cmd_tune_defaults(data *d){
	d->tnt_conf.kp = APPCONF_TNT_KP;
	d->tnt_conf.kp2 = APPCONF_TNT_KP2;
	d->tnt_conf.mahony_kp = APPCONF_TNT_MAHONY_KP;

	d->tnt_conf.startup_pitch_tolerance = APPCONF_TNT_STARTUP_PITCH_TOLERANCE;
	d->tnt_conf.startup_roll_tolerance = APPCONF_TNT_STARTUP_ROLL_TOLERANCE;
	d->tnt_conf.startup_speed = APPCONF_TNT_STARTUP_SPEED;
	d->tnt_conf.startup_click_current = APPCONF_TNT_STARTUP_CLICK_CURRENT;
	d->tnt_conf.brake_current = APPCONF_TNT_BRAKE_CURRENT;
	d->tnt_conf.is_buzzer_enabled = APPCONF_TNT_IS_BUZZER_ENABLED;
	d->tnt_conf.tiltback_constant = APPCONF_TNT_TILTBACK_CONSTANT;
	d->tnt_conf.tiltback_constant_erpm = APPCONF_TNT_TILTBACK_CONSTANT_ERPM;
	d->tnt_conf.noseangling_speed = APPCONF_TNT_NOSEANGLING_SPEED;
	d->tnt_conf.startup_pushstart_enabled = APPCONF_PUSHSTART_ENABLED;
	d->TNT_conf.startup_simplestart_enabled = APPCONF_SIMPLESTART_ENABLED;
	d->tnt_conf.startup_dirtylandings_enabled = APPCONF_DIRTYLANDINGS_ENABLED;

	d->startup_step_size = d->tnt_conf.startup_speed / d->tnt_conf.hertz;
	d->noseangling_step_size = d->tnt_conf.noseangling_speed / d->tnt_conf.hertz;
	d->startup_pitch_trickmargin = d->tnt_conf.startup_dirtylandings_enabled ? 10 : 0;
}
