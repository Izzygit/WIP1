			...
			if ((!d->stickytiltoff) && // Don't apply sticky tilt if we just left sticky tilt
			   (fabsf(d->stickytilt_maxval) < .95)) { //Check that we have not pushed beyond this limit
				if (d->stickytilton) {//if sticky tilt is activated, switch values
					if (((fabsf(d->filtered_current) < stickytilt_maxcurrent) &&
...
static float apply_rollkp(data *d){
	...
	if (SIGN(d->proportional) != SIGN(d->erpm)) { //Braking
		if (d->abs_roll_angle > d->tnt_conf.brkroll3  && d->tnt_conf.brkroll_kp3>0 && d->tnt_conf.brkroll_kp2>0) {
			kp_min = -d->tnt_conf.brkroll_kp3;
			kp_max = -d->tnt_conf.brkroll_kp3;
			scale_angle_min = d->tnt_conf.brkroll3;
			scale_angle_max = 90;
		} else if (d->abs_roll_angle > d->tnt_conf.brkroll2  && d->tnt_conf.brkroll_kp3>0 && d->tnt_conf.brkroll_kp2>0) {
			kp_min = -d->tnt_conf.brkroll_kp2;
			kp_max = -d->tnt_conf.brkroll_kp3;
			scale_angle_min = d->tnt_conf.brkroll2;
			scale_angle_max = d->tnt_conf.brkroll3;
		} else if (d->abs_roll_angle > d->tnt_conf.brkroll1  && d->tnt_conf.brkroll_kp2>0) {
		...
		if (d->abs_roll_angle > d->tnt_conf.roll3  && d->tnt_conf.roll_kp3>0 && d->tnt_conf.roll_kp2>0) {
			kp_min = d->tnt_conf.roll_kp3;
			kp_max = d->tnt_conf.roll_kp3;
			scale_angle_min = d->tnt_conf.roll3;
			scale_angle_max = 90;
		} else if (d->abs_roll_angle > d->tnt_conf.roll2  && d->tnt_conf.roll_kp3>0 && d->tnt_conf.roll_kp2>0) {
			kp_min = d->tnt_conf.roll_kp2;
			kp_max = d->tnt_conf.roll_kp3;
			scale_angle_min = d->tnt_conf.roll2;
			scale_angle_max = d->tnt_conf.roll3;
		} else if (d->abs_roll_angle > d->tnt_conf.roll1  && d->tnt_conf.roll_kp2>0) {
...
static float apply_kp(data *d) {
	...
	if (d->prop_smooth > d->tnt_conf.pitch4 && d->tnt_conf.kp4>0 && d->tnt_conf.kp3>0 && d->tnt_conf.kp2>0 && d->tnt_conf.kp1>0) {
	...
	} else if (d->prop_smooth > d->tnt_conf.pitch3 && d->tnt_conf.kp4>0  && d->tnt_conf.kp3>0 && d->tnt_conf.kp2>0 && d->tnt_conf.kp1>0) {
		kp_min = d->tnt_conf.kp3;
		kp_max = d->tnt_conf.kp4;
		scale_angle_min = d->tnt_conf.pitch3;
		scale_angle_max = d->tnt_conf.pitch4;
	} else if (d->prop_smooth > d->tnt_conf.pitch2 && d->tnt_conf.kp3>0 && d->tnt_conf.kp2>0 && d->tnt_conf.kp1>0) {
		kp_min = d->tnt_conf.kp2;
		kp_max = d->tnt_conf.kp3;
		scale_angle_min = d->tnt_conf.pitch2;
		scale_angle_max = d->tnt_conf.pitch3;
	} else if (d->prop_smooth > d->tnt_conf.pitch2 && d->tnt_conf.kp2>0 && d->tnt_conf.kp1>0) {
		kp_min = d->tnt_conf.kp1;
		kp_max = d->tnt_conf.kp2;
		scale_angle_min = d->tnt_conf.pitch1;
		scale_angle_max = d->tnt_conf.pitch2;
	} else if (d->prop_smooth > d->tnt_conf.pitch1 && d->tnt_conf.kp1>0) {
...
static void cmd_tune_defaults(data *d){
	...
	d->tnt_conf.kp1 = APPCONF_TNT_KP0;
	d->tnt_conf.kp2 = APPCONF_TNT_KP1;
	d->tnt_conf.kp1 = APPCONF_TNT_KP2;
	d->tnt_conf.kp2 = APPCONF_TNT_KP3;
	d->tnt_conf.kp1 = APPCONF_TNT_KP4;
