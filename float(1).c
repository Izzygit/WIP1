...
static float apply_rollkp(data *d){
	...
	if (SIGN(d->proportional) != SIGN(d->erpm)) { //Braking
		if (d->abs_roll_angle > d->float_conf.brkroll3  && d->float_conf.brkrollkp3>0 && d->float_conf.brkrollkp2>0) {
			kp_min = -d->float_conf.brkrollkp3;
			kp_max = -d->float_conf.brkrollkp3;
			scale_angle_min = d->float_conf.brkroll3;
			scale_angle_max = 90;
		} else if (d->abs_roll_angle > d->float_conf.brkroll2  && d->float_conf.brkrollkp3>0 && d->float_conf.brkrollkp2>0) {
			kp_min = -d->float_conf.brkrollkp2;
			kp_max = -d->float_conf.brkrollkp3;
			scale_angle_min = d->float_conf.brkroll2;
			scale_angle_max = d->float_conf.brkroll3;
		} else if (d->abs_roll_angle > d->float_conf.brkroll1  && d->float_conf.brkrollkp2>0) {
			kp_min = -d->float_conf.brkrollkp1;
			kp_max = -d->float_conf.brkrollkp2;
			scale_angle_min = d->float_conf.brkroll1;
			scale_angle_max = d->float_conf.brkroll2;
		} else {
			kp_min = 0;
			kp_max = 0;
			scale_angle_min = 0;
		...
		if (d->abs_roll_angle > d->float_conf.roll3  && d->float_conf.rollkp3>0 && d->float_conf.rollkp2>0) {
			kp_min = d->float_conf.rollkp3;
			kp_max = d->float_conf.rollkp3;
			scale_angle_min = d->float_conf.roll3;
			scale_angle_max = 90;
		} else if (d->abs_roll_angle > d->float_conf.roll2  && d->float_conf.rollkp3>0 && d->float_conf.rollkp2>0) {
			kp_min = d->float_conf.rollkp2;
			kp_max = d->float_conf.rollkp3;
			scale_angle_min = d->float_conf.roll2;
			scale_angle_max = d->float_conf.roll3;
		} else if (d->abs_roll_angle > d->float_conf.roll1  && d->float_conf.rollkp2>0) {
			kp_min = d->float_conf.rollkp1;
			kp_max = d->float_conf.rollkp2;
			scale_angle_min = d->float_conf.roll1;
			scale_angle_max = d->float_conf.roll2;
		} else {
			kp_min = 0;
			kp_max = 0;
			scale_angle_min = 0;
...
static float apply_kp(data *d) {
	...
	if (d->prop_smooth > d->float_conf.angle4 && d->float_conf.kp4>0 && d->float_conf.kp3>0 && d->float_conf.kp2>0 && d->float_conf.kp1>0)) {
		kp_min = d->float_conf.kp4;
		kp_max = d->float_conf.kp4;
		scale_angle_min = d->float_conf.pitch4;
		scale_angle_max = 90; //Any value but zero can be used because kp_min=kp_max
	} else if (d->prop_smooth > d->float_conf.angle3 && d->float_conf.kp4>0  && d->float_conf.kp3>0 && d->float_conf.kp2>0 && d->float_conf.kp1>0) {
		kp_min = d->float_conf.kp3;
		kp_max = d->float_conf.kp4;
		scale_angle_min = d->float_conf.pitch3;
		scale_angle_max = d->float_conf.pitch4;
	} else if (d->prop_smooth > d->float_conf.angle2 && d->float_conf.kp3>0 && d->float_conf.kp2>0 && d->float_conf.kp1>0) {
		kp_min = d->float_conf.kp2;
		kp_max = d->float_conf.kp3;
		scale_angle_min = d->float_conf.pitch2;
		scale_angle_max = d->float_conf.pitch3;
	} else if (d->prop_smooth > d->float_conf.angle2 && d->float_conf.kp2>0 && d->float_conf.kp1>0) {
		kp_min = d->float_conf.kp1;
		kp_max = d->float_conf.kp2;
		scale_angle_min = d->float_conf.pitch1;
		scale_angle_max = d->float_conf.pitch2;
	} else if (d->prop_smooth > d->float_conf.angle1 && d->float_conf.kp1>0) {
		kp_min = d->float_conf.kp0;
		kp_max = d->float_conf.kp1;
		scale_angle_min = 0;
		scale_angle_max = d->float_conf.pitch1;
	} else {
		kp_min = d->float_conf.kp0;
		kp_max = d->float_conf.kp0;
		scale_angle_min = 0;
		scale_angle_max = 1; //Any value but zero can be used because kp_min=kp_max
	}
